...
class DBManager:
    ...
    MOVIE_FROM_PK = [
        "SELECT title, year, duration_min, description, rating_kp, link_kp FROM movies_movie where id = $1",
    ...
    MOVIES_FROM_FILTERS = "SELECT mm.id, title, rating_kp from movies_movie mm JOIN movies_movie_country mmc " \
                          "on mm.id = mmc.movie_id JOIN movies_country mc on mmc.country_id = mc.id WHERE " \
                          "mm.year = coalesce($1, mm.year) and mc.country = coalesce($2, mc.country) " \
                          "INTERSECT " \
                          "SELECT mm.id, title, rating_kp from movies_movie mm JOIN movies_movie_genre mmg on " \
                          "mm.id = mmg.movie_id JOIN movies_genre mg on mg.id = mmg.genre_id " \
                          "WHERE mg.genre = coalesce($3, mg.genre)"
    ...
    async def get_movies_from_filters(self, year, country, genre):
        return await self.conn.fetch(self.MOVIES_FROM_FILTERS, year, country, genre)
